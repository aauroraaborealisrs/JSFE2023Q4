...
function checkInputValidity(input: HTMLInputElement) {
    ...
    if (input.id === 'firstName') {
        if (input.value.length < 3 || input.value[0] !== input.value[0].toUpperCase()) {
            input.style.borderColor = 'red';
            return false;
        }
    }
...
class LoginForm {
    ...
    addEventListeners() {
        ...
        const checkInputs = () => {
            ...
            const hasInput = Array.from(inputs as NodeListOf<HTMLInputElement>).some((input: HTMLInputElement) => input.value.trim() !== '');
            
            const regex = /^[A-Za-z-]+$/;
            if (!regex.test((document.getElementById('firstName') as HTMLInputElement).value)) {
                firstNameValid = false;
                firstNameError.textContent = 'Only English alphabet letters and hyphen are allowed.';
                formValid = false;
            } else {
                firstNameError.textContent = '';
            }
        
            if (!regex.test((document.getElementById('surname') as HTMLInputElement).value)) {
                surnameValid = false;
                surnameError.textContent = 'Only English alphabet letters and hyphen are allowed.';
                formValid = false;
            } else {
                surnameError.textContent = '';
            }
        
            let firstNameValue = (document.getElementById('firstName') as HTMLInputElement).value;
            if (firstNameValue && firstNameValue[0] !== firstNameValue[0].toUpperCase()) {
                firstNameValid = false;
                firstNameError.textContent = 'The first letter must be uppercase.';
                formValid = false;
            }

            let surnameValue = (document.getElementById('surname') as HTMLInputElement).value;
            ...
            if ((document.getElementById('firstName') as HTMLInputElement).value.length < 3) {
                firstNameValid = false;
                firstNameError.textContent = 'First name must be at least 3 characters long.';
                formValid = false;
            }
        
            if ((document.getElementById('surname') as HTMLInputElement).value.length < 4) {
                surnameValid = false;
                surnameError.textContent = 'Surname must be at least 4 characters long.';
                formValid = false;
            }

            const forms = document.querySelectorAll('.login__input') as NodeListOf<HTMLFormElement>;
